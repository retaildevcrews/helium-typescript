    ...
    public static validateMovies(query: any) {
        ...
        if ("genre" in query) {
            if (query.genre === null || query.genre === undefined || query.genre.length < 3 || query.genre.length > 20) {
                return { validated: false, message: queryErrorMessages.invalidGenreMessage };
            }
        }
        ...
        if ("year" in query) {
            ...
            if (isNaN(year) || year != query.year || year < 1874 || year > (new Date(Date.now()).getFullYear() + 5)) {
                return { validated: false, message: queryErrorMessages.invalidYearMessage};
            }
        }
        ...
        if ("rating" in query) {
            ...
            if (isNaN(rating) || rating != query.rating || rating < 0 || rating > 10) {
                return { validated: false, message: queryErrorMessages.invalidRatingMessage};
            }
        }
